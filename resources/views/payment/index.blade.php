...
@section('content')
<div class="container">
    <h4 class="grey-text text-darken-1 center">นำเข้ารายการรับจ่ายรายเดือน {{Thaimonthonly(date('m'))}}</h4>
    {{-- Search --}}
    <div class="row mb-0">
...
{{--                                    <td>{{Thaidateonly($employee->join_date)}}</td>--}}
                                    <td>
                                    <a href="{{route('payments.edit',$employee->id)}}" class="btn btn-small btn-floating waves=effect waves-light teal lighten-2"><i class="material-icons">attach_money</i></a>
                                    </td>
                                </tr>
                            @endforeach
                            @if(isset($search))
