                        ...
                        @if($payrolls->count())
                            ...
                            @foreach($payrolls as $payroll)
                                <tr>
                                    ...
                                    <td>{{Thaimonthonly($payroll->month)}}</td>
{{--                                    <td>--}}
...
{{--                                    </td>--}}
                                    <td>
                                        <div class="row mb-0">
                                            <div class="col">
                                                <a href="{{route('payrolls.show',$payroll->emp_id)}}" class="btn btn-floating btn-small waves=effect waves-light teal"><i class="material-icons">receipt_long</i></a>
                                            </div>
                                            <div class="col">
                                                <form action="{{route('payrolls.destroy',$payroll->id)}}" method="POST">
                                                    @method('DELETE')
                                                    @csrf()
                                                    <button type="submit" class="btn btn-floating btn-small waves=effect waves-light red" onclick="return confirm('คุณต้องการลบสลิปเงินเดือนของ {{$payroll->first_name}} {{$payroll->last_name}} หรือไม่?')"><i class="material-icons">delete</i></button>
                                                </form>
                                            </div>
                                        </div>
                                    </td>
                        ...
                        @else
                            ...
                            <tr>
                                <td colspan="5"><h6 class="grey-text text-darken-2 center">ไม่พบข้อมูล!</h6></td>
                            </tr>
                        @endif
                    </tbody>
                </table>
            ...
            <div class="center">
                {{$payrolls->links('vendor.pagination.default',['paginator' => $payrolls])}}
            </div>
        </div>
    </div>
    <!-- Card END -->
